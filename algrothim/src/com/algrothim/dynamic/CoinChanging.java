...
public class CoinChanging {
	...
	/**
	 * Keep input sorted. Otherwise temp[j-arr[i]) + 1 can become
	 * Integer.Max_value + 1 which can be very low negative number
	 */
	public int minCoinChangeInfinteSupply(int total, int arr[]) {
		...
		for (int i = 0; i < arr.length; i++) {
			for (int j = 1; j <= total; j++) {
				if (j >= arr[i]) {
					// be careful here. Max_val + 1 can result in very small
					// negative number.
