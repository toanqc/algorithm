...
public class ScrambledString {
	...
	private static class Index {
		...
		public boolean equals(Object o) {
			...
			if (start1 != index.start1)
				return false;
			if (end1 != index.end1)
				return false;
			if (start2 != index.start2)
				return false;
	...
	boolean isScrambled(char input1[], char input2[], int start1, int end1, int start2, int end2,
			Map<Index, Boolean> memMap) {
		...
		// if we have already calculated value for index then lets use it
		// instead of recalculating again.
		if (memMap.containsKey(index)) {
			return memMap.get(index);
		}

		// if both input from their respective start to end are same then return
		// true.
		boolean isSame = true;
		...
		// check if both input from their respective start to end have same
		// characters. If not then return false.
		...
		// all values in map should have value 0 otherwise there is character
		// mismatch. Return false in that case.
	...
	public static void main(String args[]) {
		...
		boolean result = ss.isScrambled(str1.toCharArray(), str2.toCharArray(), 0, str1.length() - 1, 0,
				str2.length() - 1, memMap);
