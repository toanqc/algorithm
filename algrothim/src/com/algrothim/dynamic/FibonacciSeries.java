 ...
 * @author tusroy
 * 
 *         Fibonacci series Given a number find the fibonacci series value for
 *         that number e.g n = 3 -> 3 n = 4 -> 5 n = 5 -> 8
 * 
 *         Solution Recursively it can calculated very easily by f(n) = f(n-1) +
 *         f(n-2) For Dp version we do not recalculate f(n-1) and f(n-2) but
 *         keep it in a and b
 * 
 *         Test cases 1) Negative number 2) 0 3) 1 4) Very high number
 * 
 */
