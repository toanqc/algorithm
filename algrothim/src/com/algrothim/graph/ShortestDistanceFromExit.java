 ...
 *         Find distance of every empty space from nearest exits in case of fire
 *         emergency.
 * 
 *         Idea is to start from every exit position and do BFS search in all 4
 *         directions and maintain the distance of every space from exit. If
 *         another exit in future iterator is closer than already calculated
 *         exit then update the distance.
 * 
 *         Space complexity is O(n*m) Time complexity is O(number of exit * m *
 *         n);
 *
 */
public class ShortestDistanceFromExit {
	...
	public int[][] findShortest(Cell input[][]) {
		...
		for (int i = 0; i < input.length; i++) {
			for (int j = 0; j < input[i].length; j++) {
				// for every exit location do a BFS starting with this exit as
				// the origin
	...
	private void setDistanceUtil(Queue<Point> q, Cell input[][], Point p, Point newPoint, int distance[][],
			boolean visited[][]) {
	...
	public static void main(String args[]) {
		...
		Cell input[][] = { { Cell.SPACE, Cell.SPACE, Cell.BLOCK, Cell.BLOCK },
				{ Cell.SPACE, Cell.SPACE, Cell.GUARD, Cell.SPACE }, { Cell.SPACE, Cell.SPACE, Cell.BLOCK, Cell.SPACE },
