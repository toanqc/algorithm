...
public class FloydWarshallAllPairShortestPath {
		...
		for (Vertex<Integer> v : graph.getAllVertex()) {
			for (Vertex<Integer> u : graph.getAllVertex()) {
				for (Vertex<Integer> v1 : graph.getAllVertex()) {
					if (getDistance(u, v1, distance) > getDistance(u, v, distance) + getDistance(v, v1, distance)) {
						updateDistance(u, v1, distance, getDistance(u, v, distance) + getDistance(v, v1, distance));
					}
				}
			}
		}
	...
	public static void main(String args[]) {
		int[][] graph = { { 0, 2, 1000, 1, 1000 }, { 1000, 0, 3, 1, 1000 }, { 1000, 1000, 0, 1000, 1000 },
				{ 3, 1000, 3, 0, 2 }, { 1000, 1000, 3, 1000, 0 }, };
