...
/**
 * http://www.careercup.com/question?id=6026101998485504 This answer two
 * questions. Group elements in size m such that in every group only unique
 * elements are possible. It also answers question where rearrange array such
 * that same elements are exactly m distance from each other
 */
class Pair {
	int num;
	int count;
...
public class GroupElementsInSizeM {
	...
	public boolean group(int input[], int m) {
		...
		for (Integer s : count.keySet()) {
			int c = count.get(s);
			// if any count is greater than len/m then this arrangement is not
			// possible
