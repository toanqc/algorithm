...
public class SmallestWindowContaingAllCharacters {
	...
	public int smallestWindow(char[] str, char[] subString) {
		...
		for (; i < str.length; i++) {
			if (subStringVisited.containsKey(str[i])) {
				...
				if (mainStringVisited.get(str[i]) == subStringVisited.get(str[i])) {
	...
	private int trimWindow(int startIndex, char[] str, int endIndex, Map<Character, Integer> mainStringVisited,
